...
Emit the class token verbatim in the JSON summary. These five tokens are the
whole vocabulary — a sensor fails closed on anything else, so do not invent a
label or use the prose name.

- `billable-skip` — only GenAI/visual assertions failed (harness limitation, not a regression).
- `product` — a concrete assertion failed on a step the diff touches (a code regression).
- `stale-test` — a selector/assertion broke because the UI intentionally changed.
- `env-data` — wrong seed data, missing precondition, or down dependency.
- `mabl-flake` — timing race or non-deterministic behavior (correlate with history).

A test that passed all concrete assertions produces no failure entry at all.

For `product` and `stale-test` failures, cross-reference the failing step against
the diff from Step 3 to localize the cause. `env-data` and `mabl-flake` count as
  ...
  "failures": [
    {
      ...
      "confidence": 0.85,
      "rerun_passed": false,
      "failing_step": "Step 7: Assert heading contains 'Dashboard'"
    }
  ]
}
```
